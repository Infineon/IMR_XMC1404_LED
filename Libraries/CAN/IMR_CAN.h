...
#define CAN_IRQ_RX_NUMBER               		IRQ3_IRQn          	/* CAN Interrupt Number Setting		-	  XMC1404 = IRQ3_IRQn */
#define CAN_IRQ_RX_MESSAGE_HANDLER         		IRQ3_Handler   		/* CAN Interrupt Handler Setting	-	  XMC1404 = IRQ3_Handler */
...
/* This library is designed to work with the IMR2 CAN identification in mind; CAN node IDs are set using DIP switches according to the selected board type;
 * To ensure the correct pins are used, the naming for each individual CAN ID pin must follow the same naming convention: CAN_ID + PinNumber
 * 		e.g. CAN_ID1, CAN_ID2, ...
 *
 * Please make sure this naming convention is followed when naming the corresponding pins in the MTB Device Configurator;
 *
 *
 * Make sure to select up to 2 message objects in the global CAN setting for the selected node in the MTB Device Configurator;
...
#define TIMER_LED_PERIOD_MATCH_EVENT_IRQN 				IRQ16_IRQn		/* Defines IRQ number of the period match event interrupt */
#define TIMER_LED_PERIOD_MATCH_EVENT_HANDLER 			IRQ16_Handler	/* Defines handler of the period match event interrupt */
