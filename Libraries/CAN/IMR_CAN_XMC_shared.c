...
uint32_t XMC_CAN_MO_Busy(XMC_CAN_MO_t* mo_ptr) {
	return (XMC_CAN_MO_GetStatus(mo_ptr) & CAN_MO_MOSTAT_TXRQ_Msk) >>
			CAN_MO_MOSTAT_TXRQ_Pos;
}

CAN_STATUS_t CAN_TX_Request(uint32_t CAN_ID, uint8_t* Target_Data,
		uint8_t Target_Data_Length) {
	// Implement TimeOut feature to prevent CAN messages getting stuck
	// on the CAN bus with no recipient
	// TimeOut can be defined in IMR2_CAN.h
